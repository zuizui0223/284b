# Ecology Letters Method proposal email v0.4
...
Our <=300-word proposal is reproduced below with one compact quantitative figure. A classical coupling counterexample makes the additional inferential layer explicit: even exact marginals `p_E=p_F=0.5` admit hard-violation probability from 0 to 0.5 under different joint couplings, so better marginal models alone cannot identify a directional biological relation. The empirical anchor contains 12 independently held-out taxa; its 283 evaluable procedure-by-area cells are repeated diagnostics rather than independent replication. Focal cross-role biological outcomes remain sealed.
...
Use the exact text in `manuscript/ECOLOGY_LETTERS_300WORD_PITCH_V0_7.md`.
