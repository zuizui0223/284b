...
|---|---|---|---|---|
| **A relation layer is needed even when upstream answers are accurate.** | Classical nonempirical separation | For `v=P(E=1,F=0)`, Fréchet–Hoeffding bounds give `max(0,p_E-p_F) <= v <= min(p_E,1-p_F)`. At exact `p_E=p_F=0.5`, the same exact marginals admit `v=0` and `v=0.5`. | Accurate marginals alone do not identify a directional biological relation; the relation semantics must be declared separately. | The coupling bounds are new mathematics; JSDMs cannot estimate joint structure. |
