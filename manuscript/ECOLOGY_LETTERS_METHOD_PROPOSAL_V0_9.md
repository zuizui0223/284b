# Ecology Letters — unsolicited Method proposal package v0.9
...
Use the exact body frozen in `ECOLOGY_LETTERS_300WORD_PITCH_V0_8.md` (295 words under the repository word-count rule).
...
### Auditable prospective freezing

The implementation now makes prospectivity inspectable rather than leaving it as prose. Each contract explicitly stores the biological relation, key space, both adapters, both adequacy gates, opening-rule class and an `opening_rule_reference` identifying the frozen calibration artifact or negative-state qualification protocol that instantiates the rule. The complete validated contract is serialized to deterministic canonical JSON and SHA-256 fingerprinted before focal opening.

`opening_rule_reference` closes an important loophole: a contract cannot retain the same fingerprint while silently switching to another empirical envelope, threshold artifact or negative-state qualification protocol. The generic freeze utility is `scripts/freeze_relation_endpoint_contract.py`; a committed synthetic example and receipt demonstrate exact round-trip reproduction. Hashing does not by itself prove that investigators were outcome-blind, but it creates a timestampable, versionable contract identity that can be compared with the object used at endpoint opening.
...
- a specific executable contract and endpoint state machine;
- deterministic contract serialization and SHA-256 fingerprints tied to frozen opening-rule references;
- a minimal executable quickstart;
...
The reference engine directly evaluates Level A/B soft and Level C directional primitives. Levels D/E are explicitly composition-only rather than being presented as generic direct evaluators.
...
The remaining desk-review risk is conceptuality. The defense is now concrete at three levels: exact marginals do not determine a hard relation; the complete relation-endpoint contract is executable and fingerprintable before opening; and the empirical/openability examples show that the machinery can both close a fresh endpoint and refuse one when the falsifying state is not identified.
