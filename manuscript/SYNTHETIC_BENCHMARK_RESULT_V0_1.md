# Synthetic negative-state benchmark result v0.2
...
To avoid confusing observation validity with biological violation prevalence, this document uses:

- `q` = key-level sensitivity `P(at least one detection | at least one true functional event)`;
- `sp` = key-level specificity;
- `a` = fraction of keys with complete, nonmissing, nonfailed observation;
- `pi` = true violation prevalence `P(F=false | E=true)`.
...
## Exact invalid-state identities
...
With dense functional opportunity, key sensitivity is approximately 0.999998 and `a=1`. Naive and gated rules coincide: false-violation probability is approximately `1.6e-6`.

### 30% missingness with otherwise near-perfect detection

Key sensitivity remains approximately 0.999998 but `a=0.70`. The naive false-violation probability becomes approximately 0.300001, whereas the gated probability remains approximately `1.1e-6` and 30% of keys are explicitly unresolved.

### 20% device/observer failure

With the same near-perfect valid-key detection but `a=0.80`, naive false-violation probability is approximately 0.200001. The gated rate is approximately `1.3e-6`; the 20% failure mass is unresolved.

### 50% incomplete-window keys

With `a=0.50`, naive false-violation probability is approximately 0.500001 even though valid-key detection is near perfect. The gated rate is below `8e-7`, with half the keys unresolved.

### Mixed qualified regime

For key sensitivity 0.8653, specificity 0.95 and `a=0.684`, the naive false-violation probability is 0.4081 and the gated rate is 0.0921. Their difference is exactly 0.316, the invalid-key mass. The naive true-violation sensitivity is 0.9658 versus 0.6498 gated; that same 0.316 apparent gain is again entirely attributable to treating invalid keys as biological negatives.
...
## A second result: calibration is not false-discovery control

The qualification gate controls whether an observation process is eligible to generate hard negative states. It does not by itself guarantee that a called violation is likely to be a true biological violation. Among gated calls on valid keys, the false-discovery fraction depends on the biological base rate:

`FDF = (1 - pi)(1 - q) / [(1 - pi)(1 - q) + pi sp]`.

For example, with `pi=0.10`, `q=0.80` and `sp=0.99`, the gated false-discovery fraction is approximately **0.645** even when `a=1`.

This does not invalidate the calibration floor. It defines its role more precisely. Calibration is an **endpoint-opening condition**, not a biological conclusion and not a posterior-error guarantee for a called violation. A future focal Level-C endpoint must separately freeze its aggregation, replication or evidentiary rule before focal values are opened.

No existing v8/v8.1 threshold is changed by this result.

## Important threshold interpretation

The current `q >= 0.80` calibration threshold is an endpoint-opening qualification floor, not a statement that a single calibrated zero is sufficient evidence for a final biological dependency violation. At the threshold and `a=1`, key-level false-negative probability can still be 0.20. A later focal endpoint must therefore freeze its own aggregation or replication rule before values are opened. The pre-field manuscript must not convert a v8/v8.1 calibration pass into a biological conclusion.

## Manuscript consequence

The benchmark supplies two method-level results independent of the retained Level-C systems:
...
> Observation-process qualification determines whether a hard-negative state may enter the test; the evidential reliability of a called violation additionally depends on biological base rates and the prospectively frozen endpoint-level decision rule.

Together these results support the Ecology Letters Method framing alongside the Level-A empirical demonstration.