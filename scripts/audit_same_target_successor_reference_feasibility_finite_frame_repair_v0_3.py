#!/usr/bin/env python3
"""v0.3 provenance gate around the unchanged repaired pre-D feasibility audit."""
...
AUDIT_VERSION = "product_b_same_target_successor_reference_feasibility_finite_frame_repair_v0.3"
...
def main() -> int:
    ...
    for p in sorted(root.rglob("contract.json")):
        ...
        if c.get("result_version") == FIT_RESULT:
            contracts.append(c)
    if len(contracts) != 47:
        raise RuntimeError(f"v0.3 pre-D gate requires 47 repaired contracts, found {len(contracts)}")
    for c in contracts:
        if c.get("finite_frame_preflight_result_version") != FRAME_RESULT:
            raise RuntimeError("v0.3 pre-D gate rejects non-v0.3 finite-frame fit")
        if c.get("finite_frame_repair_contract_version") != REPAIR_CONTRACT:
            raise RuntimeError("v0.3 pre-D gate rejects wrong repair contract")
        if c.get("v0_3_adapter_changed_frame_bytes") is not False:
            raise RuntimeError("v0.3 pre-D gate rejects changed frame bytes")
        if c.get("v0_3_adapter_changed_model_fit_body") is not False:
            raise RuntimeError("v0.3 pre-D gate rejects changed model-fit body")
        for k in (
            ...
            if c.get(k) is not False:
                raise RuntimeError(f"v0.3 pre-D gate found opened boundary: {k}")
    code = base.main()
