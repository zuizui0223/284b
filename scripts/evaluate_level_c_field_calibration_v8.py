...
def _binomial_upper_tail(successes, n, p):
    """P_p[X >= successes] for X ~ Binomial(n, p)."""
    if successes <= 0:
        return 1.0
    if successes > n:
        return 0.0
...
def exact_one_sided_lower_bound(successes, n, alpha=ONE_SIDED_ALPHA):
    ...
    This is the Beta(alpha; successes, n-successes+1) quantile computed
    without external dependencies by inverting the exact binomial upper tail.
    It implements the v8 contract's `one_sided_95_percent_exact_or_more_conservative`
    requirement directly rather than using an asymptotic score interval.
    """
    if n <= 0:
        return None
