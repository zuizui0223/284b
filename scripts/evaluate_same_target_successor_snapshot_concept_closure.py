#!/usr/bin/env python3
"""Apply rank-agnostic successor species-concept closure after taxonomy review."""
...
REVIEWS = ROOT / "results/product_b_same_target_successor_historical_name_review_v0_1.json"
CONTRACT = ROOT / "config/product_b_same_target_successor_calibration_contract_v0_1.json"
OUTPUT = ROOT / "results/product_b_same_target_successor_snapshot_concept_closure_v0_1.json"
KEYSETS = ROOT / "registry/product_b_same_target_successor_snapshot_keysets_v0_1.csv"
...
def main() -> int:
    ...
    if tuples.get("current_name_review_authorized") is not True:
        raise RuntimeError("successor tuple aggregate did not authorize review")
    ...
    for payload, flags in (
        ...
        for flag in flags:
            if payload.get(flag) is not False:
                raise RuntimeError(f"successor taxonomy-only closure boundary violated: {flag}")
    ...
    for row in reviews.get("reviews", []):
        ...
        if not name or name in review_map:
            raise RuntimeError("successor review contains blank/duplicate historical name")
        review_map[name] = CurrentConceptResolution(
            snapshot_scientific_name=name,
            review_complete=bool(row.get("review_complete")),
            resolved=bool(row.get("resolved")),
            accepted_species_name=None if row.get("accepted_species_name") is None else str(row.get("accepted_species_name")).strip() or None,
            accepted_usage_key=None if row.get("accepted_usage_key") is None else str(row.get("accepted_usage_key")).strip() or None,
            match_type=None if row.get("match_type") is None else str(row.get("match_type")).strip() or None,
    ...
    for taxon in tuples.get("taxa", []):
        ...
        decision = evaluate_successor_snapshot_species_concept_closure(
        ...
        decisions.append({
            ...
            "observed_taxonranks": list(decision.observed_taxonranks),
            "distinct_taxonomy_tuples": [asdict(row) for row in decision.distinct_taxonomy_tuples],
        })

    sampling_authorized = passed_count >= minimum
    outcome = {
        "result_version": "product_b_same_target_successor_snapshot_concept_closure_v0.1",
        "candidate_taxa_entering_closure": len(decisions),
    ...
    with KEYSETS.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=[
            "current_accepted_species", "historical_specieskey", "key_index", "historical_key_count", "closure_state"
        ])
        writer.writeheader()
        writer.writerows(key_rows)
    print(json.dumps({
        "result_version": outcome["result_version"],
        "candidate_taxa_entering_closure": outcome["candidate_taxa_entering_closure"],
