#!/usr/bin/env python3
"""Fit one Layer-1 taxon independently from two observation modes.

This worker deliberately stops before any paired cross-source statistic is
computed. Both answers use the same source-blind M geometry, spatial block
centres, target-group background rows, and environmental predictor universe.
Only response occurrences and procedure-internal selection differ by source.
"""
...
SAMPLING_RESULT = ROOT / "results/product_b_same_target_source_mode_sampling_v0_1.json"
...
def _scan_taxon(specieskey: str) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    ...
    expression = (
        (ds.field("specieskey") == str(specieskey))
    ...
    for batch in scanner.to_batches():
        ...
        for i in range(batch.num_rows):
            if str(values["specieskey"][i] or "").strip() != str(specieskey):
...
def main() -> int:
    ...
    output_dir = Path(os.environ.get("OUTPUT_DIR", f"artifacts/layer1_fit_taxon_{taxon_index}"))
    ...
    if len(registry) != 12 or not 0 <= taxon_index < 12:
        raise RuntimeError("taxon index/registry mismatch")
    ...
    result_by_name = {str(item["requested_name"]): item for item in sampling["results"]}
    if result_by_name.get(name, {}).get("state") != "source_mode_sampling_passed":
        raise RuntimeError("taxon did not pass frozen source-mode sampling gate")
    if sampling.get("paired_discordance_opened") is not False or sampling.get("model_fit_opened") is not False:
        raise RuntimeError("upstream information boundary changed")
    ...
    # Build the shared geometry after source labels have been removed. Thinning
    # again after pooling ensures observation-system record frequency cannot
    # weight the KMeans block centres or M geometry.
    ...
    source_blocks = {
        mode: assign_spatial_blocks(
            source_thin[mode]["longitude"].to_numpy(float),
            source_thin[mode]["latitude"].to_numpy(float),
            centers,
        )
        for mode in MODES
    }
    ...
    if len(raster_specs) != int(fit_contract["environmental_predictor_source"]["active_predictors_required"]):
        raise RuntimeError("CHELSA predictor count differs from frozen contract")
    ...
    # Presence environmental values are source-specific and extracted once.
    ...
    for mode in MODES:
        ...
        provenance["role"] = f"presence_{mode}"
    ...
    for m_km in [int(v) for v in fit_contract["M_km"]]:
        m_mask = occurrence_buffer_membership(
            target,
            common_geometry,
            buffer_km=float(m_km),
        )
        background = sample_target_group_background(
            ...
            focal_species=name,
            random_state=bg_seed,
        )
        if len(background) < 100:
            raise RuntimeError(f"insufficient common target-group background for M={m_km}")
            ...
            for procedure in procedures:
                ...
                try:
                    ...
                    if not np.isfinite(scores).any():
                        raise RuntimeError("sealed prediction surface contains no finite scores")
                    for comparison_row_id, score in zip(comparison_ids, scores, strict=True):
                        prediction_rows.append({
                            **base,
                            "comparison_row_id": comparison_row_id,
                            "ecological_score": None if not np.isfinite(score) else float(score),
                        })
    ...
    fit_complete = int((selected["state"] == "layer1_model_fit_sealed").sum()) if len(selected) else 0
    outcome = {
        "result_version": "product_b_same_target_source_layer1_baseline_fit_taxon_v0.1",
        "taxon": name,
        "taxon_index": taxon_index,
        "specieskey": specieskey,
        "source_modes": list(MODES),
        "M_km": [int(v) for v in fit_contract["M_km"]],
        "procedure_count": len(procedures),
        "expected_fit_cells": 2 * len(fit_contract["M_km"]) * len(procedures),
        "sealed_fit_cells": fit_complete,
        "unresolved_fit_cells": len(selected) - fit_complete,
        ...
        "occurrence_coordinates_persisted": False,
        "shared_source_blind_M_and_block_geometry": True,
        "same_background_rows_used_for_both_sources": True,
        "errors": errors,
    }
