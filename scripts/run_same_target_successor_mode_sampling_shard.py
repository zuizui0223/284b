#!/usr/bin/env python3
"""Run one successor source-mode sampling shard after concept closure.

All frozen historical specieskeys for each admitted taxon are queried together.
No key is selected or dropped using occurrence abundance. Each observation mode
must independently clear the unchanged 50 records / 30 10-km cells / 10 effective
cells gate. Raw rows, coordinates, and identifiers are never persisted.
"""
...
def _sanitize_taxon_result(name: str, keys: tuple[str, ...], mode_a: list[dict[str, object]], mode_b: list[dict[str, object]]) -> dict[str, object]:
    ...
    preflight = build_occurrence_sampling_preflight(adapted.records, taxonomy_eligible=True, thresholds=PREPROCESSING_THRESHOLDS)
...
def main() -> int:
    ...
    if closure.get("result_version") != "product_b_same_target_successor_snapshot_concept_closure_v0.1":
        raise RuntimeError("successor concept closure result required")
    if closure.get("occurrence_sampling_authorized") is not True:
        raise RuntimeError("successor concept closure did not authorize occurrence sampling")
    if int(closure.get("concept_closure_passed_taxa", -1)) < minimum:
        raise RuntimeError("successor concept closure is below frozen calibration minimum")
    if closure.get("taxonrank_used_as_exclusion_gate") is not False:
    ...
    if int(sampling["minimum_independent_records_per_mode"]) != 50:
        raise RuntimeError("successor record floor changed")
    if int(sampling["minimum_unique_10km_cells_per_mode"]) != 30:
        raise RuntimeError("successor unique-cell floor changed")
    if float(sampling["minimum_effective_cells_per_mode"]) != 10.0:
        raise RuntimeError("successor effective-cell floor changed")
    if sampling.get("all_frozen_historical_specieskeys_are_unioned_per_taxon") is not True:
        raise RuntimeError("successor all-key union rule changed")
    if sampling.get("historical_key_level_sampling_selection_forbidden") is not True:
        raise RuntimeError("successor key-level selection firewall changed")
    ...
    for row in selected:
        ...
        gc.collect()
    outcome = {
        "result_version": "product_b_same_target_successor_source_mode_sampling_shard_v0.1",
    ...
    output = ROOT / f"artifacts/product_b_same_target_successor_source_mode_sampling_shard_{shard_index}.json"
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(json.dumps(outcome, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({"shard_index": shard_index, "taxa_assigned_to_shard": len(selected), "results": len(results)}, indent=2))
    return 0
