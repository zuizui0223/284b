#!/usr/bin/env python3
"""Resolve v6 EPV001 taxonomy without opening occurrence or scope data."""
...
CONTRACT_PATH = ROOT / "config/product_b_v6_epv001_taxonomy_resolution_v0_1.json"
OUTPUT_PATH = ROOT / "artifacts/product_b_v6_epv001_taxonomy_resolution.json"
...
def _get_json(url: str, *, timeout_seconds: float = 60.0) -> Mapping[str, object]:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "zuizui0223-284b-product-b-v6-taxonomy/0.1",
        },
        method="GET",
    )
...
def _resolve(request: TaxonomyResolutionRequest) -> dict[str, object]:
    ...
    usage_payload = _get_json(
        GBIF_SPECIES_USAGE_ENDPOINT.format(usage_key=usage_key)
    )
    return asdict(
        parse_direct_taxonomy_resolution(
            request=request,
...
def main() -> int:
    ...
    if contract.get("occurrence_reads_allowed") is not False:
        raise RuntimeError("EPV001 taxonomy-only step must keep occurrence reads closed")
    if contract.get("scope_reads_allowed") is not False:
        raise RuntimeError("EPV001 taxonomy-only step must keep scope reads closed")
