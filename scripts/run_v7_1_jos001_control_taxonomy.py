#!/usr/bin/env python3
"""Resolve the frozen JOS001 replacement-host pool without occurrence access."""
...
CONTRACT_PATH = ROOT / "config/product_b_v7_1_jos001_control_taxonomy_v0_1.json"
OUTPUT_PATH = ROOT / "artifacts/product_b_v7_1_jos001_control_taxonomy.json"
...
def _get_json(url: str, *, timeout_seconds: float = 60.0) -> Mapping[str, object]:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "zuizui0223-284b-product-b-v7-1-jos-control-taxonomy/0.1",
        },
        method="GET",
    )
...
def main() -> int:
    ...
    for row in contract["requests"]:
        control_id = str(row["control_taxon_id"])
    ...
    minimum = int(contract["minimum_required_exact_accepted_controls"])
    ...
    outcome = {
        ...
        "closed_pool_size": len(contract["requests"]),
        "resolved_candidate_count": resolved,
        "minimum_required": minimum,
        "pool_state": state,
        "results": results,
        "synonym_rescues_performed": 0,
