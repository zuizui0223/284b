#!/usr/bin/env python3
"""Resolve JOS003 focal taxa against current GBIF taxonomy without occurrence reads."""
...
OUTPUT = ROOT / "artifacts/product_b_v7_2_jos003_snapshot_taxonomy.json"
REQUESTS = (
    SnapshotTaxonomyRequest("JOS003", "x", "Yucca filamentosa", "Plantae"),
    SnapshotTaxonomyRequest("JOS003", "y", "Tegeticula cassandra", "Animalia"),
)


def _read_json(url: str, timeout: float = 60.0) -> Mapping[str, object]:
    request = Request(url, headers={"Accept": "application/json", "User-Agent": "zuizui0223-284b-product-b-v7-2-taxonomy/0.1"}, method="GET")
...
def _check_preconditions() -> None:
    ...
    if not contract["new_pair_selection_allowed"] or contract["occurrence_row_reads_allowed"]:
        raise RuntimeError("JOS003 taxonomy preconditions are not closed correctly")
    with PAIR.open("r", encoding="utf-8", newline="") as handle:
        row = list(csv.DictReader(handle))[0]
    if row["taxonomy_state"] != "taxonomy_unopened" or row["occurrence_reads_performed"] != "false":
        raise RuntimeError("JOS003 taxonomy or occurrence boundary was already crossed")
...
def main() -> int:
    ...
    for request in REQUESTS:
        try:
            ...
            resolutions.append(asdict(parse_current_direct_taxonomy_resolution(request=request, match_payload=match, usage_payload=usage_payload)))
        except Exception as exc:
            errors.append({"partner": request.partner, "requested_name": request.scientific_name, "error_type": type(exc).__name__, "error_message": str(exc)})
    status = "resolved_direct_exact_current_taxonomy" if len(resolutions) == 2 and not errors else "unresolved_snapshot_taxonomy"
    outcome = {
        "result_version": "product_b_v7_2_jos003_snapshot_taxonomy_v0.1",
        "pair_id": "JOS003",
        "status": status,
