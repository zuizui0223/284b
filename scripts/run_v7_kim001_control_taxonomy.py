#!/usr/bin/env python3
"""Resolve the frozen KIM001 replacement-host pool without occurrence access."""
...
CONTRACT_PATH = ROOT / "config/product_b_v7_kim001_control_taxonomy_v0_1.json"
OUTPUT_PATH = ROOT / "artifacts/product_b_v7_kim001_control_taxonomy.json"


def _get_json(url: str, *, timeout_seconds: float = 60.0) -> Mapping[str, object]:
    request = Request(url, headers={"Accept": "application/json", "User-Agent": "zuizui0223-284b-product-b-v7-kim-control-taxonomy/0.1"}, method="GET")
...
def _resolve(name: str) -> dict[str, object]:
    request = TaxonomyResolutionRequest(
        pair_id="KIM001_CONTROL_POOL",
        partner="x",
        scientific_name=name,
        kingdom="Plantae",
        expected_key_hint="",
    )
    match_payload = _get_json(GBIF_SPECIES_MATCH_ENDPOINT + "?" + urlencode(dict(build_species_match_params(request))))
    ...
    return asdict(parse_direct_taxonomy_resolution(request=request, match_payload=match_payload, usage_payload=usage_payload))
...
def main() -> int:
    ...
    for row in rows:
        try:
            ...
            results.append({"control_taxon_id": row["control_taxon_id"], "scientific_name": row["scientific_name"], "state": "resolved_direct_exact", **resolution})
            resolved += 1
        except Exception as exc:
            results.append({"control_taxon_id": row["control_taxon_id"], "scientific_name": row["scientific_name"], "state": "excluded_unresolved_taxonomy", "error_type": type(exc).__name__, "error_message": str(exc)})

    minimum = int(contract["minimum_taxonomy_resolved_candidates_before_any_occurrence_query"])
    state = "control_taxonomy_preflight_passed" if resolved >= minimum else "unresolved_controls"
    outcome = {
