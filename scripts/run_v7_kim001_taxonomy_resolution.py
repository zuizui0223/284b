#!/usr/bin/env python3
"""Resolve v7 KIM001 taxonomy without opening occurrences."""
...
CONTRACT_PATH = ROOT / "config/product_b_v7_kim001_taxonomy_resolution_v0_1.json"
OUTPUT_PATH = ROOT / "artifacts/product_b_v7_kim001_taxonomy_resolution.json"
...
def _get_json(url: str, *, timeout_seconds: float = 60.0) -> Mapping[str, object]:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "zuizui0223-284b-product-b-v7-kim-taxonomy/0.1",
        },
        method="GET",
    )
...
def main() -> int:
    ...
    pair_state = (
        "eligible_for_control_declaration"
        if states.get("x") == states.get("y") == "resolved_direct_exact"
        else "requires_manual_or_unresolved_taxonomy_review"
    )
    outcome = {
