...
class Core19V042RepairGuards(unittest.TestCase):
    ...
    def test_reference_preserves_original_preD_eligibility_after_repair(self):
        ...
        self.assertIn("'result_version':'product_b_same_target_core19_fit_audit_v0.4'", w)
        self.assertIn("'successor_serialization_repair_version':'v0.4.2'", w)
        self.assertIn("'successor_taxa_observed':47,'successor_expected_fit_cells':2256", w)
        self.assertIn("'heldout_taxa_observed':12,'heldout_expected_fit_cells':576", w)
        self.assertIn("'heldout_fit_artifacts_inherited_from_parent':True", w)
        self.assertNotIn("'successor_sealed_fit_cells':2256", w)
        self.assertNotIn("'successor_unresolved_fit_cells':0", w)
        self.assertIn("successor_sealed_fit_cells',-1))+int(s.get('successor_unresolved_fit_cells", w)
        self.assertIn('minimum_eligible_taxa_per_procedure_M', w)
        self.assertIn('==30', w)
