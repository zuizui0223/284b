...
from product_b_v6.htr001_execution import (
    EXPECTED_CONTROL_HOSTS,
    HTR001ExecutionNotAuthorized,
    evaluate_htr001_execution_manifest,
    execute_htr001_sampling_preflight,
    host_sampling_adequate,
    require_htr001_execution_authorization,
)
from product_b_v6.witness import HostSamplingSummary


ROOT = Path(__file__).resolve().parents[1]
MANIFEST_PATH = ROOT / "config/product_b_v6_htr001_execution_manifest_v0_1.json"
SCOPE_PATH = ROOT / "config/product_b_v6_scope_resolution_htr001_v0_1.json"
...
class HTR001AuthorizationTests(unittest.TestCase):
    ...
    def test_consumed_copy_is_fail_closed(self):
        ...
        with self.assertRaises(HTR001ExecutionNotAuthorized):
            require_htr001_execution_authorization(manifest)

    def test_host_sampling_floor_is_same_frozen_v6_floor(self):
...
class HTR001ExecutionOrderingTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_focal_failure_never_opens_controls(self):
        ...
        focal = SimpleNamespace(
            preflight=SimpleNamespace(
                passed=False,
                reasons=("witness_independent_record_floor_failed",),
            )
        )
        with patch(
            "product_b_v6.htr001_execution.adapt_and_build_directed_witness_sampling_preflight",
            return_value=(None, focal),
        ):
            result = execute_htr001_sampling_preflight(
    ...
    def test_host_specificity_review_is_required_before_any_transport(self):
        calls = []
        ...
        mutated_scope = dict(self.scope)
        mutated_scope["host_specificity_review"] = {"state": "unresolved"}
